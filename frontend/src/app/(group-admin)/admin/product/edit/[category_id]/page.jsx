...
const Categorybyid = ({ params }) => {
    ...
    const submithandler = (e) => {
        ...
        axiosApiInstance.put(`category/update/${catParams.category_id}`, data).then(
            (res) => {
    ...
    useEffect(
        () => {
            const fetchCategory = async () => {
                const categoryJSON = await getCategorybyid(catParams?.category_id);
                const data = categoryJSON?.categorys;
                setcategory(data);
            };
            fetchCategory()
        }, [catParams?.category_id]
    )
