      ...
      <div className="max-w-6xl mx-auto bg-white dark:bg-[#1e2a38] shadow-xl rounded-t-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full table-auto text-sm text-left">
            ...
            <tbody>
              {Array.isArray(brandata) &&
                brandata.map((brand, index) => (
                  <tr
                    key={index}
                    className="bg-white dark:bg-[#263447] hover:bg-gray-50 dark:hover:bg-[#324158] transition"
                  >
                    <td className="px-6 py-4 border-b text-gray-800 dark:text-gray-200">
                      {index + 1}
                    </td>
                    <td className="px-6 py-4 border-b text-gray-800 dark:text-gray-200">
                      {brand.name}
                    ...
                    <td className="px-6 py-4 border-b">
                      <Status
                        statusUrl={`brand/status/${brand._id}`}
                        Status={brand.status}
                      />
                    </td>
                    <td className="px-6 py-4 border-b">
